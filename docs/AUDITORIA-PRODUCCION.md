...
- **2026-07-02 — Paso 7 (CI) ejecutado**:
  ...
  - **Verificado en local cada comando del workflow**: eslint exit 0, `tsc --noEmit` exit 0, **22/22 unit tests del frontend en ChromeHeadless**, YAML válido (yaml-lint); build/lint/jest del backend ya verificados en los pasos 5-6. **Primera corrida real (push de `a17b1c9`): ambos jobs en verde.**
...
**Con esto, los 8 pasos del plan de remediación (§5) quedan completados.** Deuda conocida restante: TLS gestionado fuera del compose (documentado en `docs/DESPLIEGUE.md` §5) y e2e fuera del CI (locales). El CI corrió en verde en su primera ejecución real (push de `a17b1c9`, 2026-07-02).
